...
\textbf{Step 1: Select a Model}
\begin{itemize}
    \item In the sidebar (left side), find the \textbf{Model Selection} dropdown
    \item Choose one of the 4 available models: Neural Network, Logistic Regression, Random Forest, or XGBoost
    \item The selected model will be used for prediction
\end{itemize}

\textbf{Step 2: Enter Customer Information}
\begin{itemize}
    \item \textbf{Demographics (Column 1):} Select gender, senior citizen status, partner, and dependents
    \item \textbf{Services (Column 2):} Choose phone service, internet service, and additional services
    \item \textbf{Account Info (Column 3):} Set tenure (months), contract type, billing, payment method, and charges
\end{itemize}

\textbf{Step 3: Get Prediction}
\begin{itemize}
    \item Click the \textbf{Predict Churn} button
    \item View the results:
    \begin{itemize}
        \item \textbf{Churn Prediction:} Yes or No
        \item \textbf{Churn Probability:} Percentage (0-100\%)
        \item \textbf{Risk Level:} Low (green), Medium (yellow), or High (red)
    \end{itemize}
\end{itemize}

\textbf{Step 4: Review Recommendations}
\begin{itemize}
    \item Based on the risk level, the app provides actionable recommendations
    \item For high-risk customers: Consider offering discounts or service upgrades
    \item For medium-risk customers: Engage with loyalty programs
    \item For low-risk customers: Continue current service
\end{itemize}

\textbf{Step 5: View Customer Profile (Optional)}
\begin{itemize}
    \item Click on \textbf{Customer Profile Summary} to expand
    \item Review all entered customer data in table format
\end{itemize}
