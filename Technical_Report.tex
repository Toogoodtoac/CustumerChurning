...
\title{\textbf{Telco Customer Churn Prediction Report}}
\author{Ngo Anh Hieu}
\date{January 2026}
...
\begin{abstract}
This report presents a comprehensive machine learning solution for predicting customer churn in the telecommunications industry. The project encompasses the complete data science pipeline, from exploratory data analysis through model deployment.
\end{abstract}
...
The objective of this project is to develop a machine learning model capable of predicting customer churn. This is formulated as a \textbf{binary classification} problem:
\begin{itemize}
    \item \textbf{Input:} Customer information (demographics, services, billing)
    \item \textbf{Output:} Churn prediction (Yes or No)
\end{itemize}

\subsection{Dataset}
This project utilizes the \textbf{IBM Telco Customer Churn} dataset with the following characteristics:
\begin{itemize}
    \item 7,043 customer records
    \item 21 features per customer
    \item 26.5\% churn rate (class imbalance)
\end{itemize}
...
Various factors were analyzed to understand what drives churn:
...
This feature aggregates the count of additional services subscribed by each customer, including online security, online backup, device protection, tech support, streaming TV, and streaming movies. The underlying hypothesis is that customers with multiple service subscriptions demonstrate higher engagement and switching costs, thereby reducing churn probability.
...
The data was split \textbf{before} applying SMOTE. If SMOTE is applied first, fake data can leak into the test set and give inflated (wrong) results!
\end{tcolorbox}
...
\begin{enumerate}
    \item \textbf{Logistic Regression:} Linear classifier with sigmoid activation
    \item \textbf{Random Forest:} Ensemble of 100 decision trees with bootstrap aggregation
    \item \textbf{XGBoost:} Gradient boosting with regularization
    \item \textbf{Neural Network:} Feedforward network with 2 hidden layers (16 and 8 neurons)
\end{enumerate}

\subsection{Loss Function and Optimization}

\textbf{Loss Function:} For this binary classification problem, all models utilize \textbf{Binary Cross-Entropy Loss} (also known as Log Loss):
...
\begin{itemize}
    ...
    \item $y_i$ = actual label (0 or 1)
    \item $\hat{y}_i$ = predicted probability of churn
\end{itemize}
...
\begin{tabular}{lll}
\toprule
\textbf{Model} & \textbf{Optimizer} & \textbf{Details} \\
\midrule
Logistic Regression & L-BFGS & Quasi-Newton method, max\_iter=1000 \\
Random Forest & Gini Impurity & Information gain criterion \\
XGBoost & Gradient Descent & learning\_rate=0.1, max\_depth=6 \\
Neural Network & Adam & lr=0.001, $\beta_1$=0.9, $\beta_2$=0.999 \\
\bottomrule
\end{tabular}
\caption{Optimization algorithms used for each model.}
\end{table}

\textbf{Neural Network Architecture:}
\begin{itemize}
    \item Input layer: 30 features (after preprocessing)
    \item Hidden layer 1: 16 neurons, ReLU activation, Dropout(0.3)
    \item Hidden layer 2: 8 neurons, ReLU activation, Dropout(0.3)
    \item Output layer: 1 neuron, Sigmoid activation
    \item Training: 50 epochs, batch\_size=32, early stopping with patience=10
\end{itemize}
...
\textbf{Recommendation:} For companies prioritizing maximum churner detection over precision, Logistic Regression is the recommended choice due to its superior recall performance, even at the cost of increased false positives.
...
A web service was created that accepts customer data and returns predictions.
...
For non-technical users, an interactive web app with rich features was developed.
...
\subsection{Application User Guide}

The web application provides an intuitive interface for churn prediction. The following workflow describes the prediction process:

\textbf{Step 1: Model Selection}

Navigate to the sidebar panel and locate the Model Selection dropdown menu. Select the desired prediction model from the available options: Neural Network, Logistic Regression, Random Forest, or XGBoost.

\textbf{Step 2: Customer Data Entry}

Complete the customer information form organized into three sections:
\begin{itemize}
    \item \textbf{Demographics:} Gender, senior citizen status, partner, and dependents
    \item \textbf{Services:} Phone service, internet service, and supplementary services
    \item \textbf{Account Information:} Tenure, contract type, billing preference, payment method, and charges
\end{itemize}

\textbf{Step 3: Prediction Execution}

Click the \textbf{Predict Churn} button to generate the prediction. The system displays:
\begin{itemize}
    \item Churn prediction (Yes/No)
    \item Churn probability (0-100\%)
    \item Risk classification: Low (green), Medium (yellow), or High (red)
\end{itemize}

\textbf{Step 4: Business Recommendations}

Based on the risk classification, the application provides targeted retention strategies:
\begin{itemize}
    \item \textbf{High Risk:} Immediate intervention with discount offers or service upgrades
    \item \textbf{Medium Risk:} Proactive engagement through loyalty programs
    \item \textbf{Low Risk:} Standard service continuation
\end{itemize}

\subsection{Preprocessing Pipeline}

During inference, the API automatically applies the following preprocessing steps:

\begin{enumerate}
    \item \textbf{Feature Engineering:} Computation of Tenure\_Group and Number\_of\_Services
    \item \textbf{Label Encoding:} Binary feature transformation (Female$\rightarrow$0, Male$\rightarrow$1; No$\rightarrow$0, Yes$\rightarrow$1)
    \item \textbf{Feature Transformation:} Application of ColumnTransformer for scaling and one-hot encoding
    \item \textbf{Model Inference:} Probability prediction using the selected model
\end{enumerate}
...
Initially, SMOTE was accidentally applied before splitting the data. This caused synthetic (fake) churners to appear in the test set.
